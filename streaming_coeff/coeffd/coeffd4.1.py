#!/usr/bin/env python3
"""coeffd4.1.py -- free-lunch-scoped QoS controller (coeffd4 + geometry fixes).

coeffd4.1 = coeffd4 with a two-phase cache flow and geometry fixes found by
replaying D3-v9 phase_c4 through masks() (2026-09-16). Layout stays disjoint:
    [streamers][absorbers][crowd]      (bit 0 on the left)
Everything after the absorber phase (MBA throttle, migration, settle) is
coeffd4's, unchanged.

PHASE 1 -- DONOR FENCING, one streamer at a time (fence_active lock):
  seed = its occupancy in ways (a verified step of its own), then narrow one
  way per verified step. A failed narrowing step widens back one way; a
  failed SEED returns the donor to the crowd (E). Rollback/re-probe/block
  rules are coeffd4's, unchanged. New donors are appended ABOVE the ones
  already compacted (C) -- coeffd4 inserted them at bit 0 and relocated the
  earlier donors (D3-v9: mg moved bit 0 -> bit 5).

OBSERVE -- after the last fence (and after each finished absorber) wait
  SETTLE_SCANS decide scans while the crowd expands into the freed ways by
  plain LRU, then judge who took them and what they did with them
  (absorbers(): MB gained vs %IPC gained since the unmanaged baseline). The
  wait precedes the stand-down test, else coeffd4 latches futile on the first
  scan, before anyone has expanded. Kept short: an absorber keeps growing and
  pushes the crowd down while we wait.

PHASE 2 -- ABSORBERS, one at a time (cap_active lock), stacked above the
  streamers in the order they are capped (coeffd4 sorted by name).
  A. seed = ceil(occupancy / WAY_MB) + CAP_SEED_MARGIN, then one way per
     verified step down to the associativity knee. coeffd4 started at the
     crowd width (13): under disjoint bands a cap of 12 PARTITIONS the crowd
     (clamped to 11, crowd left 2). Measured D3-v9: canneal 25.4 -> 7.6 MB,
     IPC 1.120 -> 1.001, committed (inside the 5% margin); cg 22.5 -> 42 MB.
     The margin exists because an exact occupancy seed IS the falsified
     static cat_cap arm (14.9 MB -> 4 ways, -7.3%/-9.0%). A failed seed
     releases the cap and retries one way wider next time.

NO RESERVATION. Per-victim reserve needs the controller to name the victim,
  which the falsified db gate cannot do; the crowd claims freed ways by LRU
  unnamed. The crowd region IS the reservation (c3.RSV_MAX_WAYS = 0).

D. GUARD-RELEASE widens one way at a time back to the seed the walk started
  from, then releases. coeffd4's fence guard compared against seed_ways()
  (current, already-shrunk occupancy, floor 2), so a 1-way donor under its
  floor jumped straight back to the full crowd; its cap guard compared against
  shared_ways(), which a seeded cap can exceed.

---- coeffd4 docstring (unchanged) ----
...
CAP_MIN_WAYS   = 2
CAP_SEED_MARGIN = 1    # 4.1-A: ways above measured occupancy for the first cap
SETTLE_SCANS   = 1     # 4.1: decide scans of natural LRU expansion observed after
                       # the last fence/cap step before a NEW absorber is picked.
                       # Short on purpose: an absorber keeps growing into the
                       # freed region and pushes the crowd down while we wait.
...
class Coeffd4(c3.Daemon):
    def __init__(self, *a, **kw):
        ...
        self.fence_seed = {}     # 4.1: donor -> width its fence walk started from
        self.fence_active = None # 4.1: donor currently being walked
        self.cap_active = None   # 4.1: absorber currently being walked
        self._settle_left = 0    # 4.1: scans to observe expansion before a NEW absorber
        self.cap_seed = {}       # 4.1: absorber -> width of its committed seed
        self.cap_seed_extra = {} # 4.1: absorber -> ways added after failed seeds
        ...
        # 4.1: NO per-victim reservation. It needs the controller to name the
        # victim, which the falsified db gate cannot do; the free lunch works
        # because the crowd claims freed ways by LRU without being named. The
        # crowd region IS the reservation. RSV_MAX_WAYS=0 makes coeffd3's
        # reserve axis inadmissible, so propose() falls through to throttle.
        c3.RSV_MAX_WAYS = 0
    ...
    def masks(self):
        ...
        for name in [n for n in self.slice_order if n in self.held]:
            ...
            lo += w
        # 4.1: absorbers stack in the order they were capped (dict insertion
        # order), like donors -- coeffd4 sorted them by NAME.
        for name, w in self.capped.items():
            if name in per:
    ...
    def try_fence(self):
        # 4.1: finish one donor before starting the next. coeffd4 re-ranked by
        # instantaneous hit rate every cycle, so two donors could interleave.
        order = self.admissible_donors()
        if self.fence_active in order:
            order.remove(self.fence_active)
            order.insert(0, self.fence_active)
        for n in order:
            t = self.tenants[n]
            if n in self.fence:
                cur = self.fence[n]
                ...
                # 4.1: the SEED (occupancy in ways) is its own verified step.
                # coeffd4 applied seed-1 directly, so the seed was never judged.
                cur = None
                new = self.seed_ways(n)
                seed = True
                self.fence_seed[n] = new
            self.fence[n] = new
            self.fence_active = n
            if n not in self.slice_order:
                # 4.1-C: append, so the new donor stacks ABOVE the ones already
                # compacted and nobody's band moves. insert(0) relocated them.
                self.slice_order.append(n)
            self.held.add(n)
            self.apply_masks()
            self._settle_left = SETTLE_SCANS
            self.act_seq += 1
            self.pending = {
            ...
            self.record("apply", step=("fence", n, new), donor=n, seed=seed,
                        donor_hit=t.hit_rate, donor_mbm=t.mbm_bps,
                        occ_mb=cmt_mb(t),
                        ways=f"{'crowd' if seed else cur}->{new}")
            return True
        return False
    ...
    def cap_seed_ways(self, name):
        """4.1-A: first cap width = ceil(occupancy / WAY_MB) + margin.

        Only the absorber's OWN exclusive ways are counted (disjoint bands).
        The margin plus any widening earned by failed seeds (cap_seed_extra)
        keeps the seed above the associativity knee; the walk then finds it.
        """
        mb = cmt_mb(self.tenants[name])
        w = int(mb / WAY_MB) + (1 if (mb % WAY_MB) else 0)
        w += CAP_SEED_MARGIN + self.cap_seed_extra.get(name, 0)
        return max(CAP_MIN_WAYS, min(self.nbits, w))

    def cap_continuing(self):
        """4.1: is the absorber being walked still walkable (no settle needed)?"""
        n = self.cap_active
        if n is None or f"cap|{n}" in self.blocked:
            return False
        if n not in [m for _, m, _ in self.absorbers()]:
            return False
        cur = self.capped.get(n) or self.cap_done.get(n)
        return cur is None or cur - 1 >= CAP_MIN_WAYS
    ...
    def try_eliminate(self):
        ...
        """
        cands = self.absorbers()
        # 4.1: finish the absorber being walked before picking another
        cands.sort(key=lambda x: x[1] != self.cap_active)
        for conv, n, d_mb in cands:
            if f"cap|{n}" in self.blocked:
                continue
            cur = self.capped.get(n) or self.cap_done.get(n)
            seed = cur is None
            if seed:
                # 4.1-A: first cap = measured occupancy + margin, not the crowd
                # width. The absorber keeps what it already holds; the rest of
                # the crowd is not partitioned away from the other tenants.
                cur = self.shared_ways()
                new = self.cap_seed_ways(n)
                if new + RSV_MIN_CROWD > cur:
                    # it already holds (nearly) the whole crowd: any disjoint
                    # cap would leave the others a sliver -- the bug-A shape
                    self.record("cap-skip", absorber=n, seed=new, crowd=cur,
                                occ_mb=cmt_mb(self.tenants[n]),
                                why="seed leaves crowd < RSV_MIN_CROWD")
                    continue
            else:
                new = cur - 1
            if new < CAP_MIN_WAYS:
            ...
            self.pending = {"step": ("cap", n, new), "victim": None, "absorber": n,
                            ...
                            "seed": seed}
            self.record("apply", step=("cap", n, new), absorber=n, seed=seed,
    ...
    def verify_cap(self, p):
        ...
        if hurt_self or floor_breach or collateral:
            ...
            if back is not None:
                ...
                # 4.1-A: a failed SEED has no committed width; uncapped is the
                # only verified-safe state. Release now, widen the next seed.
                self.capped.pop(n, None)
            self.apply_masks()
            why = ("assoc_knee" if hurt_self else
                   "floor_breach" if floor_breach else "collateral")
            if p.get("seed") and (hurt_self or floor_breach):
                # The seed was too narrow for THIS tenant: retry one way wider
                # rather than blocking it (coeffd4 released and blocked, which
                # disabled ELIMINATE for the TTL). Block only once a wider seed
                # would no longer leave the crowd its minimum.
                self.cap_seed_extra[n] = self.cap_seed_extra.get(n, 0) + 1
                nxt = self.cap_seed_ways(n)
                if nxt + RSV_MIN_CROWD > self.shared_ways():
                    self.blocked[f"cap|{n}"] = time.monotonic() + c3.BLOCK_TTL_SECS
                self.record("rollback", step=p["step"], absorber=n, why=why,
                            seed=True, next_seed=nxt,
        ...
        if p.get("seed"):
            self.cap_seed[n] = p["step"][2]
        self._cap_collat.pop(n, None)
        self.record("commit", step=p["step"], absorber=n, seed=bool(p.get("seed")),
    ...
    def rollback_fence(self, p):
        n = p["donor"]
        if p["prev_ways"] is None:
            # 4.1-E: the SEED step failed -- there is no narrower committed
            # width, so the donor goes back to the crowd (unfenced).
            self.held.discard(n)
            self.fence.pop(n, None)
            self.fence_seed.pop(n, None)
        else:
            # a narrowing step failed: widen back ONE way to the last width
    ...
    def guard_release(self):
        ...
        for n in sorted(self.held):
            ...
            # 4.1-D: compare against the seed the fence STARTED from.
            # seed_ways() is recomputed from the donor's CURRENT occupancy,
            # which the fence itself shrank (floor 2), so a 1-way donor was
            # released straight back to the full crowd instead of widened.
            if cur + 1 > self.fence_seed.get(n, self.nbits):
        ...
        for n in sorted(self.capped):
            ...
            self.record("guard-release", lever="cap", tenant=n,
                        ...
                        floor=(self.base.get(n) or {}).get("ipc"),
                        observed=self.ipc.get(n))
            # 4.1: widen back toward the verified SEED, then release. coeffd4
            # compared against shared_ways(), which under a seeded cap is often
            # narrower than the cap itself, so the first guard step released it
            # outright. On release, forget the walk so the next cap re-seeds;
            # on widen, the narrower commit is no longer known-safe.
            if cur + 1 > self.cap_seed.get(n, self.nbits):
                self.capped.pop(n, None)
                self.cap_done.pop(n, None)
                self.cap_seed.pop(n, None)
            else:
                self.capped[n] = cur + 1
                self.cap_done[n] = cur + 1
            self.blocked[f"cap|{n}"] = time.monotonic() + c3.BLOCK_TTL_SECS
            self.apply_masks()
            self.futile = False
            return True
    ...
    def propose(self):
        ...
        # 4.1: the donor fencing phase is over (or between absorbers). Let the
        # crowd expand into the freed ways by LRU and OBSERVE for SETTLE_SCANS
        # before judging who absorbed it. This sits BEFORE the stand-down test:
        # on the first scan after the last fence nobody has expanded yet, so
        # absorbers() is empty and coeffd4 would latch futile right there.
        if not donors and self._settle_left > 0 and not self.cap_continuing():
            self._settle_left -= 1
            self.record("settle-wait", left=self._settle_left,
                        occ_mb={m: round(cmt_mb(x), 1)
                                for m, x in self.tenants.items() if x.alive})
            return True
    ...
    def run(self):
        ...
        log(f"coeffd4.1 up (free-lunch-scoped, band cache model, aggressor-first): "
